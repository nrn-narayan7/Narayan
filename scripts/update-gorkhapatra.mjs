...
function stripTags(html) {
  return html
    ...
    .replace(/<[^>]+>/g, ' ')
    .replace(/&nbsp;/g, ' ')
    // Private-use-area glyphs (icon-font bullets/pencil marks etc. that
    // ride along in the raw text node rather than inside a real <tag>)
    .replace(/[\uE000-\uF8FF]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}
...
/**
 * Cheap heuristic typing for a fallback answer, so distractors are at
 * least pulled from the same *kind* of fact (a date question gets other
 * dates, a stat gets other stats) instead of any random archive answer —
 * which previously produced nonsensical mixes like a person's name as
 * the wrong option for "when did this happen?".
 */
 ...
 * @param {{text: string, type: string}[]} pool - every other real answer
 *   already in the archive, pre-classified by classifyAnswerType()
 */
function generateFallback(answer, pool) {
  const type = classifyAnswerType(answer);

  // Prefer distractors of the same type (other dates for a date question,
  // other numbers for a stat question); only widen to the whole pool if
  // there genuinely aren't enough same-type answers yet.
...
async function main() {
  ...
  const pastAnswerPool = archive.weeks
  ...
  for (const url of newArticleUrls.slice(0, 6)) {
    ...
    for (const { question, answer, extra } of qaPairs.slice(0, 20)) {
      ...
      if (!gen) gen = generateFallback(answer, pastAnswerPool);
